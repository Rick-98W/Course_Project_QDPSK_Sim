...
# Current Update - 2026-06-16 Font Policy

Status: completed.

Scope completed:

- Updated Transmitter HTML GUI CSS to use `Times New Roman` first and `SimHei` / `Microsoft YaHei` as Chinese fallback.
- Updated Transmitter Matplotlib `rcParams` to use `Times New Roman` first and Chinese fonts as fallback.

Reason:

- English and numeric text must render as Times New Roman.
- Chinese text must remain readable in SimHei.
- The previous Chinese-first Matplotlib configuration caused English plot text to render with the Chinese font.

Validation performed:

```powershell
F:\programs\miniconda3\envs\QDRSK_TX_ELB\python.exe -m py_compile config.py plotter.py web_app.py tx_pipeline.py main.py
node --check static\app.js
```
...
# Current Update - 2026-06-16 Matplotlib Mixed Fonts

Status: completed.

Scope completed:

- Updated Transmitter plot PNG title and axis-label rendering to split Latin and Chinese runs into separate Matplotlib text objects.
- Latin letters, symbols, and numeric tick labels use `Times New Roman`.
- Chinese title and axis text uses `SimHei`.
- Kept HTML GUI font policy unchanged.

Reason:

- Matplotlib does not reliably choose fonts per character inside one mixed Chinese/English string.
- A single mixed `axis.set_title()` or `axis.set_xlabel()` can cause the whole label, including English such as `QPSK`, `I`, `Q`, and `dB`, to render in the Chinese font.

Validation performed:

```powershell
F:\programs\miniconda3\envs\QDRSK_TX_ELB\python.exe -m py_compile plotter.py tx_pipeline.py web_app.py
F:\programs\miniconda3\envs\QDRSK_TX_ELB\python.exe -c "import numpy as np; from pathlib import Path; from plotter import render_eye_png, render_constellation_png, render_psd_png; out=Path('../_font_check/tx'); out.mkdir(parents=True, exist_ok=True); render_eye_png(np.random.default_rng(1).normal(size=(20,32)), out/'eye.png', 16, 'QPSK 发端干净基带眼图'); render_constellation_png(np.exp(1j*np.linspace(0,6.28,200)), out/'constellation.png', 'QDPSK 信道后采样星座图'); render_psd_png(np.linspace(-0.5,0.5,256), -60+20*np.cos(np.linspace(-3,3,256)), out/'psd.png', 'QDPSK 发端干净基带功率谱'); print(out.resolve())"
```

Notes:

- Existing PNG files keep their old rendered fonts until the transmitter regenerates plots.

# Current Update - 2026-06-16 Source Image Upload Workflow

Status: completed.

Scope completed:

- Changed TX startup state so no source image is loaded initially.
- Added PNG selection/upload through the TX HTML GUI.
- TX validates and stores the selected `64x64` PNG as the runtime source image.
- TX immediately pushes the selected reference image to RX:
  - UDP target `host:9000`
  - reference upload `http://host:9100/api/reference-image`
- TX renders channel-pre reference plots after image selection.
- TX renders channel-post constellation and eye plots only after `Send UDP`.
- Renamed visible plot labels from `干净` to `信道前`.
- Kept existing PNG filenames stable to avoid breaking frontend references.

Validation performed:

```powershell
F:\programs\miniconda3\envs\QDRSK_TX_ELB\python.exe -m py_compile config.py image_source.py tx_pipeline.py plotter.py web_app.py main.py
node --check static\app.js
```
...
```text
initial image_loaded -> False
upload raw_pic_64.png -> image_loaded True
reference_push.ok -> True
after upload send_result.sent -> False
after upload channel-post points -> 0
after Send UDP sent -> True
after Send UDP packet_count -> 4498
after Send UDP channel-post points -> 6000
```

Naming note:

- `信道前` is the preferred UI/report term.
- `干净` remains only in historical notes.

# Current Update - 2026-06-16 Fixed Source Image Rollback

Status: completed.

Scope completed:

- Removed the active TX page workflow that required selecting and loading a PNG in the browser.
- Restored the demo source image to fixed `Transmitter/raw_pic_64.png`.
- TX now loads the fixed image on startup and renders channel-pre plots immediately.
- `Send UDP` uploads the fixed reference PNG to RX, then renders channel-post plots and sends UDP IQ.
- Added a compatibility response for stale cached `POST /api/source-image` calls so old browser cache does not fail with 404.
- Disabled static-file caching in the TX HTTP server to reduce frontend/backend version mismatch during development.

Reason:

- Browser file selection is not hard, but it adds another state machine and migration surface.
- For the Orange Pi LAN demo, fixed-path startup is simpler: copy the project folder, keep `raw_pic_64.png` beside the TX code, install dependencies, run `main.py`.

# Current Update - 2026-06-16 TX plot upload to Receiver

Status: completed.

Scope completed:

- TX now packages 10 transmitter-side analysis PNGs after `Send UDP`.
- The package includes QPSK/QDPSK channel-pre PSD, eye, constellation plots and channel-post constellation/eye plots.
- TX uploads the zip package to RX at:

```text
POST http://<target_host>:<target_port + 100>/api/tx-plots
```

- TX status text reports both reference-image upload and TX-plot upload results.

Reason:

- Receiver GUI and Markdown export need to show the full TX/RX evidence set from one demo send.

# Current Update - 2026-06-16 Directory reorganization import migration

Status: completed.

Scope completed:

- User reorganized Transmitter files into package-oriented directories:
  - `core/`
  - `analysis/`
  - `web/`
  - `docs/`
- Added `__init__.py` files for package directories.
- Updated imports from flat module names to package imports.
- Updated `main.py` to import `web.web_app`.
- Updated `web/web_app.py` to load templates and static files from `web/`.
- Updated `config.py` so generated plots are under:
