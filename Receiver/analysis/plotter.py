...
class _Canvas:
    def __init__(self, title: str, subtitle: str) -> None:
        ...
        self._mixed_text(WIDTH / 2, 38, _normalize_title(title), 16, ha="center", va="center", color=TEXT)
        self._mixed_text(WIDTH / 2, 70, _normalize_subtitle(subtitle), 10, ha="center", va="center", color=MUTED)

    def axes_labels(self, x_label: str, y_label: str) -> None:
        self._mixed_text((self.left + self.right) / 2, HEIGHT - 38, x_label, 11, ha="center", va="center")
        self._mixed_text(34, (self.top + self.bottom) / 2, y_label, 11, ha="center", va="center", rotation=90)
    ...
    def legend(self) -> None:
        ...
        y = self.top + 24
        top = y - 18
        bottom = y + 26 * len(self._legend_items) - 6
        self._fill_rect_top_origin(x - 12, top, x + 166, bottom, PANEL)
        self._line_top_origin(x - 12, top, x + 166, top, GRID, linewidth=0.8)
        self._line_top_origin(x + 166, top, x + 166, bottom, GRID, linewidth=0.8)
        self._line_top_origin(x + 166, bottom, x - 12, bottom, GRID, linewidth=0.8)
        self._line_top_origin(x - 12, bottom, x - 12, top, GRID, linewidth=0.8)
        for index, (label, color) in enumerate(self._legend_items):
            yy = y + index * 26
            self._line_top_origin(x, yy, x + 28, yy, color, linewidth=2.5)
            self._mixed_text(x + 38, yy, label, 10, ha="left", va="center", color=TEXT)
    ...
    def _fill_rect_top_origin(self, left: float, top: float, right: float, bottom: float, color: str) -> None:
        y = top
        step = 3.0
        while y <= bottom:
            self._line_top_origin(left, y, right, y, color, linewidth=step + 0.5)
            y += step
    ...
    def _line_top_origin(
        ...
        self._line(x0, HEIGHT - y0, x1, HEIGHT - y1, color, alpha=alpha, linewidth=linewidth)
    ...
    def _text(
        ...
        self.fig.text(
            ...
            text,
            fontsize=size,
            fontproperties=_font_prop(family),
            ha=ha,
            va=va,
            color=color,
            rotation=rotation,
    ...
    def _mixed_text(
        ...
        if not runs:
            return
        widths = [_text_width(run, size, family) for run, family in runs]
        total = sum(widths)
        if rotation:
            if ha == "center":
                cursor = y + total / 2.0
                ...
                cursor = y
            for (run, family), width in zip(runs, widths):
                self._text(
                    x,
                    cursor,
                    run,
...
def _font_runs(text: str) -> list[tuple[str, str]]:
    ...
    current_family = None
    for char in text:
        family = LATIN_FONT if _is_latin_char(char) else CHINESE_FONT
        if current_family is None:
            current_family = family
        if family != current_family:
            runs.append(("".join(current), current_family))
            current = [char]
            current_family = family
        else:
            current.append(char)
    if current:
        runs.append(("".join(current), current_family or LATIN_FONT))
    return runs
...
def _text_width(text: str, size: int, family: str) -> float:
    if not text:
        return 0.0
    width = 0.0
    for char in text:
        width += _char_width_em(char, family)
    return width * size * DPI / 72.0


def _char_width_em(char: str, family: str) -> float:
    if char.isspace():
        return 0.32
    if not _is_latin_char(char):
        return 1.0
    if char in "ilI.,:;|!":
        return 0.28
    if char in "mwMW":
    ...
    if char.isdigit():
        return 0.50
    if family == LATIN_FONT:
        return 0.52
    return 0.58


def _font_prop(family: str) -> FontProperties:
    return LATIN_PROP if family == LATIN_FONT else CHINESE_PROP
