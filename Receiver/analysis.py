...
import config
from dsp import average_power, constellation_points, fixed_symbol_sample, matched_filter, rrc_filter
from demodulation import bits_to_bytes, demodulate_symbols
from image_output import compute_mse_psnr, write_png_rgb
from image_recovery import recover_image_lenient
from packet import CHANNEL_QDPSK, CHANNEL_QPSK
from plotter import (
...
def analyze_capture_directory(capture_dir: str | Path) -> CaptureAnalysisBundle:
    ...
    latest_report_path = write_capture_report(
        config.BASE_DIR / "reports" / "latest_capture_report.html",
        config.REFERENCE_IMAGE_PATH,
        qpsk_report,
        qdpsk_report,
        plots,
    ...
    index_path = write_capture_index(
        config.BASE_DIR / "reports" / "index.html",
