...
import config
from analysis.analysis import CaptureAnalysisBundle, analyze_capture_directory
from analysis.export_markdown import export_capture_markdown
from core.network_info import local_ipv4_addresses, tx_target_hint
...
class ReceiverServiceState:
    ...
    tx_target_hint: str | None = None
    reference_available: bool = False
    reference_path: str = str(config.REFERENCE_IMAGE_PATH)
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "tx_target_hint": self.tx_target_hint,
...
class ReceiverService:
    ...
    def __init__(
        ...
        self._state = ReceiverServiceState(
            ...
            tx_target_hint=tx_target_hint(self.listen_port),
            reference_available=reference_available(),
    ...
    def start(self) -> None:
        with self._lock:
            ...
            self._state.lan_addresses = local_ipv4_addresses()
            self._state.tx_target_hint = tx_target_hint(self.listen_port)
    ...
    def export_latest_capture(self) -> Path:
        with self._lock:
            latest_capture_dir = self._state.latest_capture_dir
        if latest_capture_dir is None:
            raise FileNotFoundError("service has no completed capture yet")
        export_path = export_capture_markdown(latest_capture_dir)
        with self._lock:
            self._state.latest_export_path = str(export_path)
            self._state.latest_assets["markdown_export"] = str(export_path)
            self._state.last_error = None
        return export_path
    ...
    def _store_analysis(self, bundle: CaptureAnalysisBundle) -> None:
        with self._lock:
            ...
            self._state.latest_export_path = None
