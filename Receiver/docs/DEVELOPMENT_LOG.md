...
## 2026-06-16 Round 22: pywebview desktop shell

Status: completed.

Scope completed:

- Confirmed `pywebview` is installed in `QDRSK_RX_PC`.
- Added `desktop_app.py`.
- Added reusable `create_http_server()` helper in `web_app.py`.
- `desktop_app.py` starts:
  - Receiver UDP service
  - local HTTP dashboard backend
  - pywebview window pointed at `http://127.0.0.1:9100`
- `main.py` remains available as browser debug mode.
- Updated `README.md` and `DEMO_RUNBOOK.md`.

Validation performed:

```powershell
F:\programs\miniconda3\envs\QDRSK_RX_PC\python.exe -m py_compile config.py packet.py frame_buffer.py capture.py dsp.py analysis.py demodulation.py image_recovery.py image_output.py reference_image.py report.py plotter.py udp_receiver.py receiver_service.py web_app.py desktop_app.py main.py tools\offline_reassembly_check.py tools\analyze_latest_capture.py
```

Backend smoke test without opening a GUI window:

```text
ReceiverDesktopApp.start_backend()
GET http://127.0.0.1:9100/api/state -> 200 application/json
ReceiverDesktopApp.stop_backend()
```

## 2026-06-16 Round 23: mixed English/Chinese font policy

Status: completed.

Scope completed:

- Updated Receiver GUI CSS font stack to use `Times New Roman` first and `SimHei` as Chinese fallback.
- Added `MIXED_FONT_FAMILY` in `config.py`.
- Updated Receiver Matplotlib plot text to use the mixed font stack for titles, labels, and legends.
- Kept numeric tick labels on `Times New Roman`.
...
## 2026-06-16 Round 24: Matplotlib plot mixed-font rendering

Status: completed.

Scope completed:

- Updated Receiver plot PNG text rendering to split Latin and Chinese runs into separate Matplotlib text objects.
- Latin letters, symbols, and numeric tick labels use `Times New Roman`.
- Chinese title, subtitle, axis, and legend text uses `SimHei`.
- Removed `TextPath` width measurement from Receiver plot text layout because the RX Matplotlib environment can fail in low-level font/path code.
- Fixed Receiver manual-artist legend coordinates so legend lines, border, and text align in the same top-origin coordinate system.

Validation performed:

```powershell
F:\programs\miniconda3\envs\QDRSK_RX_PC\python.exe -m py_compile plotter.py config.py report.py analysis.py
F:\programs\miniconda3\envs\QDRSK_RX_PC\python.exe -c "import numpy as np; from pathlib import Path; from plotter import render_spectrum_png, render_phase_png; out=Path('../_font_check/rx'); out.mkdir(parents=True, exist_ok=True); x=np.exp(1j*np.linspace(0,20,4096)); y=np.exp(1j*np.linspace(0.2,20.2,4096)); render_spectrum_png(out/'spectrum.png', x, y, 'RX SPECTRUM', 'matched filter IQ spectrum'); render_phase_png(out/'phase.png', x[::16], y[::16], 'RX PHASE', 'fixed-timing symbol phase'); print(out.resolve())"
```

Notes:

- Matplotlib `font.family` fallback alone is not reliable for mixed Chinese/English plot labels.
- Existing PNG files keep their old rendered fonts until a new capture/report regenerates them.

## 2026-06-16 Round 25: LAN Receiver readiness

Status: completed.

Scope completed:

- Changed Receiver UDP bind from `127.0.0.1:9000` to `0.0.0.0:9000` so Orange Pi TX can send over the phone-hotspot LAN.
- Kept the Receiver HTTP / pywebview dashboard local-only at `127.0.0.1:9100`.
- Added `network_info.py` to enumerate non-loopback local IPv4 addresses without third-party dependencies.
- Added `lan_addresses` and `tx_target_hint` to Receiver service state.
- Updated the dashboard to show:
  - TX target address
  - PC LAN IP list
  - UDP bind address
- Updated README, runbook, handoff, and overall plan for phone-hotspot LAN testing.

Validation performed:

```powershell
F:\programs\miniconda3\envs\QDRSK_RX_PC\python.exe -m py_compile config.py packet.py frame_buffer.py capture.py dsp.py analysis.py demodulation.py image_recovery.py image_output.py reference_image.py report.py plotter.py network_info.py udp_receiver.py receiver_service.py web_app.py desktop_app.py main.py tools\offline_reassembly_check.py tools\analyze_latest_capture.py
node --check static\app.js
F:\programs\miniconda3\envs\QDRSK_RX_PC\python.exe -c "from receiver_service import ReceiverService; s=ReceiverService(); state=s.snapshot(); print(state['listen_host'], state['listen_port']); print(state['lan_addresses']); print(state['tx_target_hint'])"
```
...
- Start Receiver desktop app on the PC.
- Connect Orange Pi and PC to the same phone hotspot.
- Set TX target to the Receiver dashboard `TX 目标地址`.
- If no UDP packets arrive, check Windows Firewall inbound UDP `9000`.

## 2026-06-16 Round 26: TX-uploaded reference image

Status: completed.

Scope completed:

- Receiver no longer reads the original image from `Transmitter/raw_pic_64.png`.
- Added `Receiver/reference/tx_reference.png` as the runtime reference image path.
- Added `POST /api/reference-image` to receive the selected TX source PNG.
- Added reference-image availability to `/api/state`.
- Receiver GUI now shows whether the reference image has been received.
- `/reference-image` serves the uploaded reference image and returns `404` before upload.
- Analysis now fails explicitly if TX has not uploaded the reference PNG.
- Receiver HTTP binds `0.0.0.0:9100` so Orange Pi TX can upload the reference image over LAN.
- pywebview still opens `http://127.0.0.1:9100`.

Validation performed:

```powershell
F:\programs\miniconda3\envs\QDRSK_RX_PC\python.exe -m py_compile config.py reference_image.py reference_store.py analysis.py receiver_service.py web_app.py desktop_app.py main.py
node --check static\app.js
```

HTTP reference upload smoke test:

```text
POST http://127.0.0.1:19100/api/reference-image -> ok true, 64x64 RGB
GET  http://127.0.0.1:19100/api/state -> reference_available true
```

Deployment note:

- Windows Firewall must allow UDP `9000` for IQ packets and TCP `9100` for TX reference-image upload.

## 2026-06-16 Round 27: Fixed TX reference source

Status: completed.

Scope completed:

- Kept Receiver behavior unchanged: RX waits for TX to upload the reference PNG at `POST /api/reference-image`.
- Updated Receiver docs/runbook to match the simplified TX workflow.
- TX now uses fixed `Transmitter/raw_pic_64.png`; the browser no longer selects the source file.

Reason:

- RX still must not read the Transmitter directory directly.
- Fixed TX source keeps Orange Pi migration simpler while preserving the correct PC RX / Orange Pi TX data ownership boundary.

## 2026-06-16 Round 28: Markdown export from Receiver GUI

Status: completed.

Scope completed:

- Added Receiver Markdown export for the latest analyzed capture.
- Export excludes the original reference image by design.
- Export includes:
  - QPSK recovered PNG
  - QDPSK recovered PNG
  - all Receiver-side communication analysis plots
  - QPSK / QDPSK quality and analysis parameter comparison table
- Added `POST /api/export-latest`.
- Added `导出 Markdown` button and export link in the Receiver GUI.
- The capture-local file is written as `analysis_export.md`.
- The latest copy is written to `Receiver/exports/latest_analysis_export.md`.

Reason:

- Markdown is dependency-free and easy to copy into Word/WPS/Typora.
- `.docx` export would require adding a new dependency such as `python-docx`, which is not needed for the current demo workflow.

## 2026-06-16 Round 29: TX plot sync into Receiver GUI and export

Status: completed.

Scope completed:

- Removed the Receiver GUI `打开导出` link; the GUI now only generates Markdown.
- Added RX endpoint `POST /api/tx-plots` for a TX-uploaded zip package.
- RX validates and stores the 10 transmitter-side analysis PNGs.
- RX copies those TX plots into the latest capture after analysis.
- Receiver GUI displays the 10 TX plots at the bottom in the same order as the TX page.
- Markdown export now includes the 10 TX plots in addition to recovered images and RX-side plots.

Reason:

- Opening Markdown should be controlled by the user through Typora or the file manager, not forced through the browser route.
- TX plots are part of the demo evidence and should live with the RX capture/export artifact.

## 2026-06-16 Round 30: Directory reorganization import migration

Status: completed.

Scope completed:

- User reorganized Receiver files into package-oriented directories:
  - `core/`
  - `analysis/`
  - `storage/`
  - `web/`
  ...
  - `runtime/`
  - `scratch/`
- Added `__init__.py` files for package directories.
- Updated Receiver imports from flat module names to package imports.
- Updated entry points:
  - `main.py`
  - `desktop_app.py`
  - `tools/analyze_latest_capture.py`
  - `tools/offline_reassembly_check.py`
- Updated runtime paths in `config.py`:
  - captures -> `runtime/captures`
  - reports -> `runtime/reports`
  - reference -> `runtime/reference`
  - exports -> `runtime/exports`
  - TX plots -> `runtime/tx_plots`
- Updated web static/template lookup to use `web/static` and `web/templates`.
- Updated README path references.
- Rewrote `docs/HANDOFF.md` so a future session can resume from that file alone.
- Rewrote `docs/OVERALL_PLAN.md` to make Orange Pi LAN testing the next active phase.
...
JS check passed
offline reassembly check passed
latest capture analysis passed
HTTP server construction smoke passed
manual user tests passed after reorganization
```
